...
_prompt_zimtheme_main() {
  # This runs in a subshell
  RETVAL=${?}
  BG_COLOR=

  print -n "╭─%B%(!.%F{red]}."
  _prompt_zimtheme_status
  _prompt_zimtheme_pwd
  _prompt_zimtheme_git
  _prompt_zimtheme_end
}
...
_prompt_zimtheme_standout_segment() {
  print -n "%S%F{${1}}"
...
_prompt_zimtheme_status() {
  ...
  if [[ -n ${segment} ]]; then
    _prompt_zimtheme_segment ${STATUS_COLOR} ${segment}' '
  fi
}
...
_prompt_zimtheme_pwd() {
  ...
  _prompt_zimtheme_standout_segment ${PWD_COLOR} ' '${current_dir}' '
}
...
_prompt_zimtheme_git() {
  if [[ -n ${git_info} ]]; then
    ...
    _prompt_zimtheme_standout_segment ${git_color} ' '${(e)git_info[prompt]}' '
  fi
}
